...
// ---------------------------------------------------------------------------
// Comms inbox (ported from apps/api messaging.service.ts). Stock-api has no
// comms.service, so we reproduce the SAME inbox the compliance-api exposes so
// the identical unread/action-required threads surface here — and crucially so
// the synthetic `comms-${thread.id}` ids match byte-for-byte.
// ---------------------------------------------------------------------------
...
function commsActorVenue(actor: AuthUser): string | null {
  return actor.deviceAccount?.venue ?? actor.venue ?? null;
}

// Mirrors apps/api messaging.service.ts canManageMessaging so manager users see
// the full inbox exactly like compliance-api does.
function canManageMessaging(actor: AuthUser): boolean {
  return Boolean(
...
// Mirrors apps/api messaging.service.ts actorRecipientWhere.
function actorRecipientWhere(actor: AuthUser): Array<Record<string, unknown>> {
  const scopedVenue = commsActorVenue(actor);
  const parts = [
...
// Mirrors apps/api messaging.service.ts threadSummary (the fields the comms
...
function threadSummary(
  thread: {
    ...
    priority: string;
    updatedAt: Date;
    recipients: Array<{ staffProfileId: string | null; readAt: Date | null; actionRequired: boolean }>;
...
// Ported from apps/api comms.service.ts listCommsInbox -> listInboxForUser.
async function listCommsInbox(actor: AuthUser): Promise<CommsInboxItem[]> {
...
export const notificationsService = {
  async list(actor: AuthUser): Promise<SuiteNotification[]> {
    ...
    const commsThreads = await listCommsInbox(actor).catch(() => [] as CommsInboxItem[]);
