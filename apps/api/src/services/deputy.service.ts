...
export async function syncRoster(connection: IntegrationConnection, options: RosterSyncOptions = {}) {
  ...
  // (e.g. an overnight shift): they were never deleted but re-created each
  // run, so duplicates piled up. Anchoring delete to the same field the
  // create is keyed on keeps the two in sync.
...
async function runSyncForRoute(actor: AuthUser, trigger: SyncTrigger, task: 'roster' | 'employees' | 'documents' | 'timesheets' | 'all') {
  ...
  try {
    ...
    // 'all' — run in employee → documents → roster → timesheets order so
