// Loaded migration imports — mirrors apps/stock-api/src/services/loaded-import.service.ts
// so the admin-web Loaded import page can call this API directly through its
// existing /api/* Firebase rewrite without crossing origins to stock-api.
//
...
function assertCanImport(actor: AuthUser) {
  if (!(actor.isAdmin || actor.role === 'ADMIN')) {
    throw new HttpError(403, 'Loaded import is admin-only on this surface.');
  }
}

export const loadedImportService = {
  async previewItemImport(actor: AuthUser, csv: string) {
    ...
    const report = rows.map((raw, index) => {
      ...
      const name = pick(raw, 'item', 'item name', 'name', 'product').trim();
      const categoryName = pick(raw, 'category', 'group') || null;
      const unit = pick(raw, 'purchase unit', 'purchaseunit', 'unit') || 'each';
      const countUnit = pick(raw, 'count unit', 'countunit') || null;
      const conversionText = pick(raw, 'conversion', 'conversion factor', 'factor');
      const conversionFactor = parseNumber(conversionText) ?? 1;
      const countArea = pick(raw, 'area', 'count area', 'location') || null;
      const costText = pick(raw, 'cost', 'latest cost', 'price', 'unit cost');
      const latestCostCents = parseCostToCents(costText);
      const activeText = pick(raw, 'active', 'status').toLowerCase();
      ...
      const lookup = byName.get(name.toLowerCase());
      const proposed = { name, categoryName, unit, countUnit, conversionFactor, countArea, latestCostCents, active };

      if (!name) {
        return { csvRow, name: '', matchedItemId: null, action: 'error' as Action, warnings, reason: 'Row has no item name.', proposed };
      }
      const dedupeKey = name.toLowerCase();
      if (seenNames.has(dedupeKey)) {
        duplicateNames.push(name);
        return { csvRow, name, matchedItemId: null, action: 'skip' as Action, warnings, reason: 'Duplicate within CSV.', proposed };
      }
      seenNames.add(dedupeKey);
      return {
        csvRow, name,
        matchedItemId: lookup?.id ?? null,
        action: lookup ? ('update' as Action) : ('create' as Action),
        warnings,
        proposed
      };
    });

    const summary: Record<Action, number> = { create: 0, update: 0, skip: 0, error: 0 };
    for (const row of report) summary[row.action] += 1;
    return { rows: report, summary, duplicateNames };
  },

  async commitItemImport(actor: AuthUser, csv: string) {
    assertCanImport(actor);
  ...
  async previewStocktakeImport(actor: AuthUser, csv: string) {
    ...
    rows.forEach((raw, index) => {
      ...
      existing.lines.push({ csvRow, itemName, matchedItemId: match?.id ?? null, category, area, quantity, unit, valueCents, costCents });
  ...
  async commitStocktakeImport(actor: AuthUser, csv: string, options: { skipUnmatched: boolean }) {
    assertCanImport(actor);
