...
const AUTH_TOKEN_KEY = 'alma.compliance.session';
...
export async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(
      `Cannot reach the ALMA API at ${API_BASE_URL}. Check that the API server is running and the frontend API URL is correct.`,
      0
    );
  }
...
/** Build a full URL to the API; used for direct links to downloadable endpoints. */
