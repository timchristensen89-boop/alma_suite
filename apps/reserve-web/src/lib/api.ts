...
const AUTH_TOKEN_KEY = 'alma.reserve.session';
...
export async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(
      `Cannot reach the ALMA Reserve API at ${API_BASE_URL}. Check that the API server is running and the frontend API URL is correct.`,
      0
    );
  }
