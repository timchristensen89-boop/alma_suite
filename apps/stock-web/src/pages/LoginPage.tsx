...
export function LoginPage() {
  ...
  const forgotPasswordHref = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(
    'ALMA Stock password reset'
  )}&body=${encodeURIComponent(
    `Hi, I need help resetting my ALMA Stock password.\n\nEmail: ${email.trim()}`
  )}`;
  const requestAccessHref = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(
    'ALMA Stock access request'
  )}&body=${encodeURIComponent(
    `Hi, I need access to ALMA Stock.\n\nEmail: ${email.trim()}\nVenue:\nRole:`
  )}`;

  return (
    <div className="login-wrap stock-login-wrap">
      <div className="login-card">
        <div className="login-brand">
          <ProductLogo appId="stock" size="lg" />
        </div>

        <Card title="Sign in" subtitle="Use your Alma account for stock control">
        ...
        <SuiteAppSwitcher currentApp="stock" apps={suiteApps} />
      </div>
    </div>
  );
}
