...
# Read-only audit of the staff register (where the 376 come from) against the production database.
...
cd "$(dirname "$0")/.."
DATABASE_URL="$TCP_URL" pnpm --filter @alma/api exec tsx scripts/staff-archive-terminated.ts
