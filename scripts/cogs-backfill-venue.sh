...
# Backfill venue on unassigned Xero bills (DRY-RUN unless BACKFILL_CONFIRM=YES) against the production database.
...
echo "→ Running Xero bill venue backfill..."
cd "$(dirname "$0")/.."
DATABASE_URL="$TCP_URL" pnpm --filter @alma/api exec tsx scripts/cogs-backfill-venue.ts
