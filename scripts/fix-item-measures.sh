#!/usr/bin/env bash
set -euo pipefail
# Repair item measurePerCountUnit from the real pack size in each name (DRY RUN unless FIX_MEASURE_CONFIRM=YES).
INSTANCE="alma-compliance:australia-southeast1:alma-compliance-db"; PORT=5449; PROJECT="alma-compliance"
...
echo "→ Repairing item measures (${FIX_MEASURE_CONFIRM:-DRY RUN})..."
cd "$(dirname "$0")/.."
DATABASE_URL="$TCP_URL" FIX_MEASURE_CONFIRM="${FIX_MEASURE_CONFIRM:-}" \
  pnpm --filter @alma/api exec tsx scripts/fix-item-measures.ts
echo "✓ Done."
