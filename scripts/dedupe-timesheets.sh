...
# Remove duplicate Xero-import timesheets that double a Deputy/manual shift
# (DRY RUN unless DEDUPE_TIMESHEETS_CONFIRM=YES). Mirrors migrate-production.sh
# (Secret Manager + Cloud SQL proxy).
...
echo "→ Running..."
cd "$(dirname "$0")/.."
DATABASE_URL="$TCP_URL" DEDUPE_TIMESHEETS_CONFIRM="${DEDUPE_TIMESHEETS_CONFIRM:-}" pnpm --filter @alma/api exec tsx scripts/dedupe-timesheets.ts
echo "✓ Done."
