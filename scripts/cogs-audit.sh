...
# Read-only audit of Xero supplier bills feeding COGS against the production database.
...
cd "$(dirname "$0")/.."
DATABASE_URL="$TCP_URL" pnpm --filter @alma/api exec tsx scripts/cogs-audit.ts
